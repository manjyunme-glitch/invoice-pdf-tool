...
class ReleaseMetadataTests(unittest.TestCase):
    def test_version_has_one_python_source_of_truth(self):
        self.assertEqual(__version__, "6.1.1")
        self.assertEqual(APP_VERSION, "v6.1.1")
        self.assertEqual(WINDOWS_EXE_BASENAME, "invoice-pdf-tool-v6.1.1-windows-x64")
        parser = build_parser()
    ...
    def test_release_files_agree_on_executable_name_and_version(self):
        ...
        for relative_path in (
            ...
            self.assertIn("6.1.1", text, relative_path)
